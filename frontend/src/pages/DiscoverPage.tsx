...
import { Sparkles, RefreshCw, Star, ArrowRight, ArrowLeft, Globe, Plus } from 'lucide-react'
import { Link } from 'react-router-dom'
import { useMoodSuggestions } from '@/hooks/useMovies'
import type { MoodAnswers, MovieSuggestion } from '@/types'
...
const QUESTIONS = [
  {
    ...
    options: [
      ...
      { value: 'thrilling and intense', label: 'Thriller',     emoji: '😱' },
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'TV series or web series',         label: 'TV / Series',  emoji: '📺' },
      { value: 'animated movie or animated show', label: 'Animation',    emoji: '🎨' },
      { value: 'documentary film',               label: 'Documentary',  emoji: '🎙️' },
      { value: 'any type',                       label: 'Any type',     emoji: '🌐' },
    ],
  },
  {
    id: 'era',
  ...
  },
  {
    ...
    options: [
      ...
      { value: '90 to 120 minutes', label: '90 – 120 min', emoji: '⏱️' },
...
export function DiscoverPage() {
  ...
  const [allSuggestions, setAllSuggestions] = useState<MovieSuggestion[]>([])
  const [submitted, setSubmitted] = useState(false)

  const { mutate, isPending, isError, reset } = useMoodSuggestions()
  ...
  function pick(value: string) {
    ...
    if (step < QUESTIONS.length - 1) {
      ...
      setSubmitted(true)
      mutate({ answers: updated as MoodAnswers }, {
        onSuccess: (data) => setAllSuggestions(data),
      })
    }
  }

  function loadMore() {
    const exclude = allSuggestions.map((s) => s.movie.title)
    mutate({ answers: answers as MoodAnswers, exclude }, {
      onSuccess: (data) => setAllSuggestions((prev) => [...prev, ...data]),
    })
  }
  ...
  const q = QUESTIONS[step]
  const showResults = submitted && !isPending && allSuggestions.length > 0
      ...
      <div className="max-w-3xl mx-auto px-4 pt-28 pb-20">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-cinema-muted font-body text-sm">
            Answer {QUESTIONS.length} quick questions and Claude picks movies just for you.
          </p>
        </div>
              ...
              >
                ...
                {step > 0 && (
                  <div className="flex justify-center mt-6">
                    <button onClick={back} className="flex items-center gap-1.5 text-sm text-cinema-muted hover:text-cinema-text font-body transition-colors">
        ...
        {/* ── Loading (initial or more) ── */}
        {isPending && allSuggestions.length === 0 && (
          <div className="flex flex-col items-center gap-4 py-20">
            ...
            <p className="text-cinema-muted/40 font-body text-xs">Picking movies just for you</p>
          </div>
        )}

        {/* ── Error ── */}
        {isError && allSuggestions.length === 0 && (
        ...
        {showResults && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            ...
            {/* Loading more spinner — inline below existing results */}
            {isPending && (
              <div className="flex items-center justify-center gap-2 py-6 text-cinema-muted/60 font-body text-sm">
            ...
            {!isPending && (
              <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
                ...
                >
                  <Plus size={15} /> Show me more
                </button>
                <button
                  onClick={restart}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-cinema-navy-border hover:border-cinema-muted/30 text-cinema-muted hover:text-cinema-text font-body text-sm transition-all"
                >
        ...
        {submitted && !isPending && allSuggestions.length === 0 && !isError && (
          <div className="text-center py-16">
            <p className="text-cinema-muted font-body mb-4">No results found — try different preferences.</p>
